...
**2. Plan groups.** For each `ai: updated prompt` commit (only the file `ai/query.md`), assign it as `fixup` under its nearest code commit. Flag mislabeled commits (wrong message for actual file content).
