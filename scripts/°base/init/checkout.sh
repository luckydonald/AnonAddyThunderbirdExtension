...
HOOK_PATH='export PATH="$HOME/.local/bin:$HOME/.cargo/bin:$HOME/.pyenv/shims:/opt/homebrew/bin:/usr/local/bin:$PATH"'
...
add_path_to_pre_commit_hooks() {
  ...
  for hook_name in pre-commit commit-msg; do
    ...
    grep -q 'AllMyStorage hook PATH' "$hook_file" && continue
